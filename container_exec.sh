...
RESPONSE=`docker run --mount type=bind,src="$(pwd)/../",target=/workspace -w /workspace -p 127.0.0.1:8080:8080/tcp --name c-cpp-sandbox-build -d c-cpp-sandbox:latest 2>&1`
if [ $? -ne 125 ]; 
then
    ...
    docker run --mount type=bind,src="$(pwd)/../",target=/workspace -w /workspace -p 127.0.0.1:8080:8080/tcp --name c-cpp-sandbox-build -d c-cpp-sandbox:latest 2>&1
fi
echo "EXECUTING THE FOLLOWING COMMAND IN THE DOCKER CONTAINER: $@"
# # exec command on running container
# BUILD_SCRIPT=$1; shift
docker exec -it c-cpp-sandbox-build bash -c "cd /workspace && $@"